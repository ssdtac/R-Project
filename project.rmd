...
As we can see, the adjusted R-squared value is 0.013, which implies that plotting these 2 variables against each other has almost no predictive validity. This makes sense if you think about it, so now we will try to find something more predictably valid.
...
```{r}
autoplot(less, series="averageRating") + labs(x="Release Year", y="Average Movie Rating") + autolayer(fitted()) 
```
